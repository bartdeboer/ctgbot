...
func main() {
	...
	resp, err := clientpkg.Do(context.Background(), getenv("HOSTBRIDGE_ADDR", "host.docker.internal:4567"), getenv("HOSTBRIDGE_TLS_DIR", ""), hostbridgev2.Request{Request: req})
	if err != nil {
		fmt.Fprintln(os.Stderr, "error:", err)
		os.Exit(1)
	}
...
func printHelp() {
	fmt.Fprintln(os.Stdout, "usage: hostbridgev2 <command> [args...]")
