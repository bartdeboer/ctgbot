	...
	r.Routes(func(b *clir.Builder) {
		b.Handle("hostbridge serve", "Serve the hostbridge controller over TCP", func(req *clir.Request) error {
			...
			if strings.TrimSpace(resolvedTLSDir) == "" {
				return hostbridge.Serve(ctx, *addr, *timeoutSec, allow.Commands(), nil, logger)
			}
			...
			return hostbridge.ServeListener(ctx, ln, *timeoutSec, hostbridge.StaticAllowedCommandResolver(allow.Commands()), nil, logger)
		})
	})
}
