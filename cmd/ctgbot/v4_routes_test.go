...
func TestV4ProfileSetAndComponentRegister(t *testing.T) {
	withTempCwd(t, func(root string) {
		...
		router := clir.New()
		registerV4Routes(router, store)

		profileOutput := captureStdout(t, func() {
			if err := router.Run(context.Background(), []string{"v4", "profile", "set", "work", "--runtime", "local", "--home-path", "profiles/work-root"}); err != nil {
		...
		registerOutput := captureStdout(t, func() {
			if err := router.Run(context.Background(), []string{"v4", "component", "register", "codex/work", "--profile", "work"}); err != nil {
		...
		rt, err := v4runtime.Open(context.Background(), v4runtime.OpenOptions{Store: store})
		if err != nil {
			t.Fatalf("open v4 runtime: %v", err)
		}
		componentRow, err := rt.Storage.Components().GetByTypeAndName(context.Background(), "codex", "work")
		if err != nil {
...
func TestV4ChatComponentAddBindsExternalChatID(t *testing.T) {
	withTempCwd(t, func(root string) {
		_ = root
		store, err := clistate.NewCwd("ctgbot", "config")
		if err != nil {
		...
		if err := router.Run(context.Background(), []string{"v4", "component", "register", "telegram", "--profile", "default"}); err != nil {
			t.Fatalf("component register: %v", err)
		}
		if err := router.Run(context.Background(), []string{"v4", "chat", "create", "team"}); err != nil {
		...
		chats, err := rt.Storage.Chats().List(context.Background())
		if err != nil {
		...
		bindOutput := captureStdout(t, func() {
			if err := router.Run(context.Background(), []string{"v4", "chat", chats[0].ID.String(), "component", "add", "source", "telegram", "--external-chat-id", "chat-1"}); err != nil {
		...
		bindings, err := rt.Storage.ChatComponents().ListEnabledByChatID(context.Background(), chats[0].ID)
		if err != nil {
