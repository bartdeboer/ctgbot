...
func registerConfigRoutes(r *clir.Router, store *clistate.Store, globalStore *clistate.Store) {
	...
	r.Routes(func(b *clir.Builder) {
		b.Handle("config", "Show or update ctgbot config", func(req *clir.Request) error {
			...
			fmt.Println("    enable a chat with: ctgbot config chat <internal-chat-id> --set-enabled true")
			fmt.Println("    set chat process tools with: ctgbot config chat <internal-chat-id> --set-process-tools-enabled true")
			fmt.Println("    set a chat workspace with: ctgbot config chat <internal-chat-id> --set-workspace-host-path <path>")
			fmt.Println("    set a chat hostbridge command with: ctgbot config chat <internal-chat-id> hostbridge <alias> --set-command <command>")
			fmt.Println("    set a chat hostbridge dir with: ctgbot config chat <internal-chat-id> hostbridge <alias> --set-dir <dir>")
			fmt.Println("    set a chat hostbridge args with: ctgbot config chat <internal-chat-id> hostbridge <alias> --set-args arg1,arg2")
			fmt.Println("    remove a chat hostbridge alias with: ctgbot config chat <internal-chat-id> hostbridge <alias> --remove true")
			fmt.Println("    add a chat skill with: ctgbot config chat <internal-chat-id> --add-skill <absolute-skill-dir>")
			fmt.Println("    remove a chat skill with: ctgbot config chat <internal-chat-id> --remove-skill <absolute-skill-dir>")
			return nil
		})
