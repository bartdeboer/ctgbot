...
COPY go.mod go.sum ./
RUN go mod download
RUN GOBIN=/usr/local/bin go install github.com/bartdeboer/go-supervisor/cmd/supervisor@v0.0.3 \
    && GOBIN=/usr/local/bin go install github.com/bartdeboer/go-supervisor/cmd/supervisord@v0.0.3
