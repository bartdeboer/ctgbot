...
func TestSandboxPropagatesConfiguredUser(t *testing.T) {
	...
	uid := 0
	gid := 0
	runtime := factory.Bind(registration, profile, runtimepkg.BindConfig{UID: &uid, GID: &gid}).(*Runtime)
...
func TestSandboxPropagatesConfiguredGPUs(t *testing.T) {
	...
	profile := factory.ComponentProfile(registration)
	runtime := factory.Bind(registration, profile, runtimepkg.BindConfig{GPUs: "all"}).(*Runtime)
...
func TestSandboxPropagatesConfiguredUnconfinedSeccomp(t *testing.T) {
	...
	registration := coremodel.Component{Type: "mockagent", Name: "default", Runtime: "docker"}
	profile := factory.ComponentProfile(registration)
	runtime := factory.Bind(registration, profile, runtimepkg.BindConfig{Seccomp: "unconfined"}).(*Runtime)
...
func TestSandboxRejectsUnsupportedSeccompMode(t *testing.T) {
	...
	registration := coremodel.Component{Type: "mockagent", Name: "default", Runtime: "docker"}
	profile := factory.ComponentProfile(registration)
	runtime := factory.Bind(registration, profile, runtimepkg.BindConfig{Seccomp: "strict"}).(*Runtime)
