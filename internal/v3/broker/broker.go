...
type InstanceResolver interface {
	ResolveComponent(ctx context.Context, componentID modeluuid.UUID) (*v3component.Instance, error)
}
...
func (b *Broker) HandleInbound(ctx context.Context, event v3component.InboundEvent) (EventOutcome, error) {
	...
	if sourceBinding == nil {
		b.logf("v3 inbound dropped component=%s external_chat=%q reason=no-source-binding", event.ComponentID, externalChatID)
	...
	for _, agentBinding := range runtime.Agents {
		...
		result, err := agentBinding.Agent.HandleTurn(ctx, v3component.Turn{
			Chat:    *chat,
			Thread:  *thread,
			Inbound: *inbound,
			Runtime: turnRuntime,
