...
func (f *captureBindFactory) ComponentHome(registration coremodel.Component) v5runtime.Home {
	_ = registration
	return f.home
}
func (f *captureBindFactory) RuntimeComponentHomePath(registration coremodel.Component, home v5runtime.Home) string {
...
func (f *captureBindFactory) Bind(registration coremodel.Component, home v5runtime.Home, config v5runtime.BindConfig) v5runtime.Runtime {
	_, _ = registration, home
	f.config = config
	return &capturedRuntime{home: f.home}
}

type capturedRuntime struct{ home v5runtime.Home }

func (r *capturedRuntime) Kind() string                                          { return "docker" }
func (r *capturedRuntime) ComponentHome() v5runtime.Home                         { return r.home }
...
func (r *capturedRuntime) Start(context.Context, string, modeluuid.UUID) (v5runtime.Status, error) {
	return v5runtime.Status{}, nil
}
...
func (r *capturedRuntime) Status(context.Context, string, modeluuid.UUID) (v5runtime.Status, error) {
	return v5runtime.Status{}, nil
}
...
func TestNewUsesProfileRuntimeConfigAndProfileDefaults(t *testing.T) {
	withTempCwd(t, func(root string) {
		...
		if err := os.WriteFile(filepath.Join(homePath, v5runtime.ConfigFilename), []byte(`{
  ...
  "reasoning_effort": "high"
		...
		factory := &captureBindFactory{home: v5runtime.Home{Path: homePath}}
...
func TestNewFallsBackToGlobalDockerImageWhenProfileImageUnset(t *testing.T) {
	withTempCwd(t, func(root string) {
		...
		if err := os.WriteFile(filepath.Join(homePath, v5runtime.ConfigFilename), []byte(`{"gpus":"all"}`), 0o644); err != nil {
			t.Fatalf("WriteFile(runtime.json) error = %v", err)
		}
...
func TestNewPrefersExplicitImageOverrideOverProfileAndGlobal(t *testing.T) {
	withTempCwd(t, func(root string) {
		...
		if err := os.WriteFile(filepath.Join(homePath, v5runtime.ConfigFilename), []byte(`{"image":"ctgbot-codex:gpu"}`), 0o644); err != nil {
			t.Fatalf("WriteFile(runtime.json) error = %v", err)
		}
