...
func (c *Component) loadState() (mailboxState, error) {
	...
	if err != nil {
		...
		return mailboxState{}, fmt.Errorf("read gmail state %s: %w", path, err)
	}
...
func (c *Component) saveState(state mailboxState) error {
	...
	if err != nil {
		return fmt.Errorf("encode gmail state: %w", err)
	}
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, append(data, '\n'), 0o600); err != nil {
		return fmt.Errorf("write gmail state: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("commit gmail state: %w", err)
	}
	return nil
}
