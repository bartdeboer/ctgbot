...
//
// It is intentionally owned by the Gmail component: when an agent runs
// `hostbridge gmail/work message ...`, ctgbot invokes this command surface on
// the gmail/work component and the handler calls Gmail's SendMessage method.
...
func buildMessageCommand(req *clir.Request) (any, error) {
	fields, err := parseMessageFields("gmail message", req.Extra)
	if err != nil {
		return nil, err
	}
