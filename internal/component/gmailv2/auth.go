...
var (
	errMissingAuthMaterial      = errors.New("missing gmailv2 auth material")
	errMissingGmailToken        = errors.New("gmailv2 token not found")
	errMissingOAuthClientConfig = errors.New("gmailv2 oauth client config not found")
)
...
func (c *Component) Auth(ctx context.Context, callbackPort int, callbackTimeout time.Duration, stdout io.Writer, stderr io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("open gmailv2 oauth callback listener: %w", err)
	}
	...
	fmt.Fprintf(stdout, "gmailv2 oauth client: %s\n", configPath)
	...
	if err != nil {
		return fmt.Errorf("exchange gmailv2 oauth code: %w", err)
	}
	...
	fmt.Fprintf(stdout, "gmailv2 auth completed\naccount: %s\n", firstNonEmpty(c.mailboxEmail, c.userID()))
	return nil
}
...
func (c *Component) AuthStatus(ctx context.Context, stdout io.Writer, stderr io.Writer) error {
	...
	if err != nil {
		if isMissingAuthMaterial(err) {
			fmt.Fprintf(stdout, "gmailv2 auth: not authenticated\n%s\n", gmailMissingAuthHelp(c, err))
			return nil
		}
		return err
	}
	...
	c.mailboxEmail = strings.TrimSpace(profile.EmailAddress)
	fmt.Fprintf(stdout, "gmailv2 auth: authenticated\naccount: %s\n", firstNonEmpty(c.mailboxEmail, c.userID()))
	return nil
}
...
func (c *Component) loadOAuthConfig() (*oauth2.Config, string, error) {
	for _, path := range c.oauthConfigPaths() {
		...
		if err != nil {
			...
			return nil, path, fmt.Errorf("read gmailv2 oauth client config %s: %w", path, err)
		}
		config, err := google.ConfigFromJSON(data, oauthScopes...)
		if err != nil {
			return nil, path, fmt.Errorf("parse gmailv2 oauth client config %s: %w", path, err)
		}
...
func (c *Component) loadToken() (*oauth2.Token, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read gmailv2 token %s: %w", path, err)
	}
...
func (c *Component) saveToken(token *oauth2.Token) error {
	if token == nil {
		return fmt.Errorf("missing gmailv2 token")
	}
	path := c.tokenPath()
	...
	if err != nil {
		return fmt.Errorf("encode gmailv2 token: %w", err)
	}
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, append(data, '\n'), 0o600); err != nil {
		return fmt.Errorf("write gmailv2 token: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("commit gmailv2 token: %w", err)
	}
	return nil
}
...
func oauthCallbackHandler(wantState string, codeCh chan<- string, errCh chan<- error) http.Handler {
	...
	mux.HandleFunc("/oauth2callback", func(w http.ResponseWriter, req *http.Request) {
		...
		if got := query.Get("state"); got != wantState {
			...
			sendCallbackError(errCh, fmt.Errorf("invalid gmailv2 oauth state"))
			return
		}
		if oauthErr := strings.TrimSpace(query.Get("error")); oauthErr != "" {
			http.Error(w, oauthErr, http.StatusBadRequest)
			sendCallbackError(errCh, fmt.Errorf("gmailv2 oauth error: %s", oauthErr))
			return
		}
		code := strings.TrimSpace(query.Get("code"))
		if code == "" {
			http.Error(w, "missing oauth code", http.StatusBadRequest)
			sendCallbackError(errCh, fmt.Errorf("missing gmailv2 oauth code"))
			return
		}
...
func gmailOAuthConfigHelp(c *Component) string {
	...
	return "gmailv2 oauth client config missing; create a Google OAuth Desktop client and save its JSON at " + paths
}
...
func gmailMissingAuthHelp(c *Component, err error) string {
	...
	default:
		return "gmailv2 auth material missing; run auth status again after installing managed files"
	}
}
