...
type Component struct {
	ID        modeluuid.UUID `gorm:"primaryKey"`
	Type      string         `gorm:"index;uniqueIndex:idx_v4_component_type_name"`
	Name      string         `gorm:"uniqueIndex:idx_v4_component_type_name"`
	Profile   string
	Label     string
	Enabled   bool
	IsDefault bool
...
type ChatComponent struct {
	ID             modeluuid.UUID    `gorm:"primaryKey"`
	ChatID         modeluuid.UUID    `gorm:"index;uniqueIndex:idx_v4_chat_component_role"`
	ComponentID    modeluuid.UUID    `gorm:"uniqueIndex:idx_v4_chat_component_role"`
	Role           ChatComponentRole `gorm:"uniqueIndex:idx_v4_chat_component_role"`
...
type ThreadComponentMapping struct {
	ID                modeluuid.UUID `gorm:"primaryKey"`
	ThreadID          modeluuid.UUID `gorm:"index;uniqueIndex:idx_v4_thread_component_binding"`
	ChatID            modeluuid.UUID `gorm:"index;uniqueIndex:idx_v4_component_thread_mapping"`
	ComponentID       modeluuid.UUID `gorm:"uniqueIndex:idx_v4_thread_component_binding;uniqueIndex:idx_v4_component_thread_mapping"`
	ComponentThreadID string         `gorm:"index;uniqueIndex:idx_v4_component_thread_mapping"`
