...
func (r *Runtime) EnsureComponent(ctx context.Context, ref string, profileName string) (*coremodel.Component, error) {
	if r == nil || r.Storage == nil {
		return nil, fmt.Errorf("missing runtime storage")
	}
	if r.Registry == nil {
	...
	if componentRow == nil {
		...
		if _, err := r.Profiles.Resolve(profileName); err != nil {
			return nil, err
		}
	...
	profileName = strings.TrimSpace(profileName)
	if profileName == "" {
		if strings.TrimSpace(componentRow.Profile) == "" {
			componentRow.Profile = "default"
		}
	} else {
		if _, err := r.Profiles.Resolve(profileName); err != nil {
			return nil, err
		}
		componentRow.Profile = profileName
	}
