...
func validateNode(node *Node, rootKind NodeKind) error {
	...
	if node.Kind == "" {
		return fmt.Errorf("node kind is empty")
	}
	switch node.Kind {
	case NodeDocument:
	...
	case NodeParagraph:
		if node.Text != "" {
			return fmt.Errorf("paragraph node cannot carry text")
		}
		for _, child := range node.Children {
	...
	case NodeList:
		if node.Text != "" {
			return fmt.Errorf("list node cannot carry text")
		}
		for _, child := range node.Children {
	...
	case NodeListItem:
		if node.Text != "" {
			return fmt.Errorf("list_item node cannot carry text")
		}
		for _, child := range node.Children {
			if !isBlockKind(child.Kind) {
	...
	case NodeStrong, NodeEmphasis:
		if node.Text != "" {
			return fmt.Errorf("%s node cannot carry text", node.Kind)
		}
		for _, child := range node.Children {
			if !isInlineKind(child.Kind) {
...
func isBlockKind(kind NodeKind) bool {
	switch kind {
	case NodeParagraph, NodeCodeBlock, NodeList, NodeListItem:
		return true
	default:
		return false
	}
