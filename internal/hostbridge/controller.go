...
func Serve(ctx context.Context, address string, defaultTimeoutSec int, allowed map[string]AllowedCommand, sendFile SendFileHandler, sendText SendTextHandler, logger *log.Logger) error {
	return server.Serve(ctx, address, defaultTimeoutSec, allowed, sendFile, sendText, logger)
}
...
func ServeListener(ctx context.Context, ln net.Listener, defaultTimeoutSec int, resolve AllowedCommandResolver, sendFile SendFileHandler, sendText SendTextHandler, logger *log.Logger) error {
	return server.ServeListener(ctx, ln, defaultTimeoutSec, resolve, sendFile, sendText, logger)
}
...
func handleConn(conn net.Conn, resolve AllowedCommandResolver, sendFile SendFileHandler, sendText SendTextHandler, defaultTimeoutSec int, logger *log.Logger) {
	server.HandleConn(conn, resolve, sendFile, sendText, defaultTimeoutSec, logger)
}

func DefaultAllowedCommands() map[string]AllowedCommand { return server.DefaultAllowedCommands() }
func MergeAllowedCommands(extra map[string]string) map[string]AllowedCommand { return server.MergeAllowedCommands(extra) }
...
func MergeAllowedCommandSpecs(specs []string) map[string]AllowedCommand { return server.MergeAllowedCommandSpecs(specs) }
func AllowedCommandsFromSpecs(specs []string) map[string]AllowedCommand { return server.AllowedCommandsFromSpecs(specs) }
func AllowedCommandNames(allowed map[string]AllowedCommand) []string { return server.AllowedCommandNames(allowed) }
