...
func (s *Service) ThreadConfigList(ctx context.Context, actor coremodel.Actor, threadID modeluuid.UUID) ([]ThreadConfigValue, error) {
	if err := s.ensureStorage(); err != nil {
		return nil, err
	}
	if err := requireActor(actor); err != nil {
	...
	config, err := ParseThreadConfig(thread.ConfigJSON)
	if err != nil {
		return nil, err
	}
...
func (s *Service) ThreadConfigGet(ctx context.Context, actor coremodel.Actor, threadID modeluuid.UUID, key string) (ThreadConfigValue, error) {
	if err := s.ensureStorage(); err != nil {
		return ThreadConfigValue{}, err
	}
	if err := requireActor(actor); err != nil {
		return ThreadConfigValue{}, err
	}
	...
	value, ok := config.Value(key)
	if !ok {
		return ThreadConfigValue{}, UnknownThreadConfigKey(key)
	}
	return ThreadConfigValue{Key: key, Value: value}, nil
}

func (s *Service) ThreadConfigSet(ctx context.Context, actor coremodel.Actor, threadID modeluuid.UUID, key string, value string) (ThreadConfigValue, error) {
	if err := s.ensureStorage(); err != nil {
		return ThreadConfigValue{}, err
	}
	if err := requireActor(actor); err != nil {
	...
	if err := s.Storage.Transaction(ctx, func(tx repository.Storage) error {
		...
		key = normalizeThreadConfigKey(key)
		stored, ok := config.Value(key)
		if !ok {
			return UnknownThreadConfigKey(key)
		}
		blob, err := config.JSON()
		if err != nil {
			return err
		}
		thread.ConfigJSON = blob
		if err := tx.Threads().Save(ctx, thread); err != nil {
			return err
		}
		out = ThreadConfigValue{Key: key, Value: stored}
		return nil
	}); err != nil {
		return ThreadConfigValue{}, err
	}
