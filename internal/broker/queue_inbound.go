...
func (b *Broker) QueueResolvedInbound(ctx context.Context, inbound component.ResolvedInbound) error {
	...
	go func() {
		if _, err := b.HandleResolvedInbound(runCtx, queued); err != nil {
			b.logf("queued inbound failed chat=%s thread=%s err=%v", queued.Chat.ID, queued.Thread.ID, err)
		}
	}()
	return nil
}
